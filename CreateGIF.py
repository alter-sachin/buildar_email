...
class GIF:
	...
	def addSubtitle(self, subtitle_text, inputVideoFile, outputVideoFile):
		generator = lambda txt: TextClip(
		    txt, font='Arial', fontsize=12, color='white')
		...
		result = CompositeVideoClip(
		    [video, subtitles.set_position((0.03, 0.85), relative=True)])

		result.write_videofile(str(outputVideoFile), fps=video.fps, temp_audiofile="temp-audio.m4a",
	...
	def convertVideoToGifFile(self):
	    ...
	    os.system("ffmpeg -i " + inputFile +
	              " -vcodec libx265 -crf 28 " + inputFileOptimised + " -y")
	    os.system("ffmpeg -i " + inputFileOptimised +
	              " -vf scale=256:-1 " + inputFileResized + " -y")
	    os.system("ffmpeg -i " + inputFileResized +
	              " -filter:v fps=10 " + inputFileFrames + " -y")
	    ...
	    os.system("rm *.mp4")
	    os.system("gifsicle -O4 " + outputFile +
	              " -o " + outputgif + " --colors 128")
	    self.upload_gif(outputFile, "/home/ubuntu/buildar_webapp/distribution/images")
	    return outputFile

	def upload_gif(self,outputFile, remoteurl):
		k = paramiko.RSAKey.from_private_key_file("../buildar.pem")
		c = paramiko.SSHClient()
		c.set_missing_host_key_policy(paramiko.AutoAddPolicy())
		print("connecting")
		c.connect( hostname = "ec2-13-234-112-28.ap-south-1.compute.amazonaws.com", username = "ubuntu", pkey = k )
		print("connected")
		sftp = c.open_sftp()
		sftp.put(outputFile, os.path.join(remoteurl, outputFile))
		sftp.close()
